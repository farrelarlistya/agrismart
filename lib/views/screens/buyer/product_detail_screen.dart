...
class _ProductDetailScreenState extends State<ProductDetailScreen> with SingleTickerProviderStateMixin {
    ...
    return Scaffold(backgroundColor: AppColors.background, body: Stack(children: [
      CustomScrollView(slivers: [
        ...
        SliverToBoxAdapter(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _buildProductInfo(p), _buildSellerInfo(p), _buildQuantitySelector(), _buildTabSection(p), const SizedBox(height: 100),
        ])),
      ]),
      _buildBottomBar(p),
    ]));
  ...
  Widget _buildSliverAppBar(Product p) {
    return SliverAppBar(expandedHeight: 260, pinned: true, backgroundColor: AppColors.white,
      ...
      actions: [GestureDetector(onTap: () => setState(() => _isFavorite = !_isFavorite), child: Container(margin: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.1), blurRadius: 8)]), child: Padding(padding: const EdgeInsets.all(8), child: Icon(_isFavorite ? Icons.favorite : Icons.favorite_border, color: _isFavorite ? AppColors.red : AppColors.grey, size: 20))))],
      flexibleSpace: FlexibleSpaceBar(background: Stack(fit: StackFit.expand, children: [
        Image.asset(p.imageUrl, fit: BoxFit.cover, errorBuilder: (context, error, stackTrace) => Container(color: AppColors.greyLight, child: Icon(Icons.eco, size: 100, color: AppColors.primary.withOpacity(0.2)))),
        if (p.isOrganic) Positioned(bottom: 16, left: 16, child: Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: AppColors.primary, borderRadius: BorderRadius.circular(20)), child: const Text('🌿 ORGANIK', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: Colors.white)))),
      ])));
  }
  ...
  Widget _buildProductInfo(Product p) {
    return Container(color: AppColors.white, padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: Text(p.name, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: AppColors.textPrimary)))]),
      ...
      Row(children: [
        const Icon(Icons.star, color: Color(0xFFFFC107), size: 16), const SizedBox(width: 4),
        Text('${p.rating}', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
        Text(' (${p.reviewCount} ulasan)', style: AppTextStyles.bodySmall),
        const Spacer(),
    ...
    return Container(color: AppColors.white, margin: const EdgeInsets.only(top: 8), child: Column(children: [
      ...
      SizedBox(height: 160, child: TabBarView(controller: _tabController, children: [
        ...
        Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _InfoRow('Kategori', p.category), _InfoRow('Satuan', p.unit), _InfoRow('Stok', '${p.stock} ${p.unit}'), _InfoRow('Rating', '${p.rating} / 5.0'),
        ])),
      ])),
    ]));
  }
  ...
  Widget _buildBottomBar(Product p) {
    return Positioned(bottom: 0, left: 0, right: 0, child: Container(
      ...
      child: Row(children: [
        Expanded(child: OutlinedButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CartScreen())), style: OutlinedButton.styleFrom(side: const BorderSide(color: AppColors.primary), padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppDimens.radiusL))), child: const Text('Keranjang', style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.w600)))),
        const SizedBox(width: 12),
        Expanded(flex: 2, child: PrimaryButton(text: 'Beli Sekarang →', onPressed: () {})),
      ]),
    ));
  }
}
