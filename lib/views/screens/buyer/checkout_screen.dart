...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  final product = AppData.products[9];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _buildAddressSection(), const SizedBox(height: 12),
        _buildOrderSection(), const SizedBox(height: 12),
        _buildShippingSection(), const SizedBox(height: 12),
        _buildPaymentSection(), const SizedBox(height: 12),
        _buildSummarySection(), const SizedBox(height: 12),
        _buildNoteField(), const SizedBox(height: 20),
        _buildCheckoutButton(), const SizedBox(height: 16),
      ])),
    );
  }

  Widget _buildAddressSection() {
    return _SectionCard(title: 'Alamat Pengiriman', trailing: TextButton(onPressed: () {}, child: const Text('+ Tambah Baru', style: TextStyle(fontSize: 12, color: AppColors.primary))),
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Icon(Icons.location_on, color: AppColors.primary, size: 18), const SizedBox(width: 10),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Rumah (Farmer John)', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
          const SizedBox(height: 4),
          const Text('Jl. Kebun raya No. 45, Desa Hijau, Kecamatan Asri, Bogor, 16751', style: TextStyle(fontSize: 12, color: AppColors.textSecondary, height: 1.5)),
          const SizedBox(height: 8),
          Row(children: [_SmallButton(label: 'Hapus', onTap: () {}, isOutline: true), const SizedBox(width: 8), _SmallButton(label: 'Simpan', onTap: () {})]),
        ])),
      ]));
  }

  Widget _buildOrderSection() {
    return _SectionCard(title: 'Pesanan Anda', trailing: Text('AgriFresh Bandung', style: const TextStyle(fontSize: 12, color: AppColors.textSecondary)),
      child: Row(children: [
        ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.asset(product.imageUrl, width: 50, height: 50, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Container(width: 50, height: 50, decoration: BoxDecoration(color: AppColors.greenBadge, borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.eco, color: AppColors.primary, size: 26)))),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(product.name, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
          const SizedBox(height: 2),
          Text('1 kg · ${formatPrice(product.price)}', style: AppTextStyles.bodySmall),
        ])),
  ...
  Widget _buildSummarySection() {
    return _SectionCard(title: 'Ringkasan Pembayaran', child: Column(children: [
      _SummaryRow('Subtotal Harga', formatPrice(_subtotal)), const SizedBox(height: 8),
      _SummaryRow('Total Ongkos Kirim', formatPrice(_shipping)), const SizedBox(height: 12),
      const Divider(color: AppColors.divider), const SizedBox(height: 8),
  ...
  Widget _buildNoteField() {
    return Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: AppColors.white, borderRadius: BorderRadius.circular(AppDimens.radiusL)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Catatan', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textPrimary)), const SizedBox(height: 8),
  ...
  Widget _buildCheckoutButton() {
    return Column(children: [
      Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: const Color(0xFFFFF3E0), borderRadius: BorderRadius.circular(10)),
        child: Row(children: [const Icon(Icons.info_outline, color: AppColors.orange, size: 16), const SizedBox(width: 8),
          Expanded(child: Text('Saya menyetujui Syarat & Ketentuan yang berlaku di AgriSmart.', style: const TextStyle(fontSize: 11, color: AppColors.textSecondary, height: 1.4)))])),
  ...
  void _showSuccessDialog() {
    showDialog(context: context, barrierDismissible: false, builder: (_) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        PrimaryButton(text: 'Lihat Pesanan', onPressed: () { Navigator.pop(context); Navigator.pop(context); Navigator.pop(context); }, height: 44),
      ]),
    ));
  }
}
...
class _SmallButton extends StatelessWidget {
  final String label; final VoidCallback onTap; final bool isOutline;
  const _SmallButton({required this.label, required this.onTap, this.isOutline = false});
  @override
  Widget build(BuildContext context) {
    return GestureDetector(onTap: onTap, child: Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 5),
      decoration: BoxDecoration(color: isOutline ? Colors.transparent : AppColors.primary, border: Border.all(color: isOutline ? AppColors.grey : AppColors.primary), borderRadius: BorderRadius.circular(20)),
      child: Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: isOutline ? AppColors.textSecondary : Colors.white))));
  }
}
