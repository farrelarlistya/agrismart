...
class DrawerScreen extends StatelessWidget {
      ...
      Expanded(child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4), child: GestureDetector(onTap: () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (_) => const SellerMainScreen())); },
          child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 16), decoration: BoxDecoration(color: AppColors.primary, borderRadius: BorderRadius.circular(AppDimens.radiusM)),
            child: Row(children: const [Icon(Icons.store, color: Colors.white, size: 18), SizedBox(width: 10), Text('Buat Toko', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w600)), Spacer(), Icon(Icons.arrow_forward_ios, color: Colors.white, size: 14)])))),
        const SizedBox(height: 12),
        _buildSection('APLIKASI LAIN', [
          _DrawerItem(icon: Icons.notifications_outlined, label: 'Notifikasi', onTap: () => Navigator.pop(context)),
  ...
  Widget _buildHeader(BuildContext context) {
    return Padding(padding: const EdgeInsets.all(16), child: Row(children: [
      Container(width: 50, height: 50, decoration: BoxDecoration(color: AppColors.greenBadge, shape: BoxShape.circle), child: const Icon(Icons.person, color: AppColors.primary, size: 28)),
      const SizedBox(width: 12),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: const [
        Text('Muh Hanif', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
        Text('muh.hanif@email.com', style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
      ])),
...
class _DrawerItem extends StatelessWidget {
  final IconData icon; final String label; final VoidCallback onTap; final Color? iconColor; final Color? textColor; final String? trailing;
  const _DrawerItem({required this.icon, required this.label, required this.onTap, this.iconColor, this.textColor, this.trailing});
  @override
  Widget build(BuildContext context) {
    return ListTile(leading: Icon(icon, color: iconColor ?? AppColors.textSecondary, size: 20), title: Text(label, style: TextStyle(fontSize: 14, color: textColor ?? AppColors.textPrimary, fontWeight: FontWeight.w500)),
      trailing: trailing != null ? Text(trailing!, style: const TextStyle(fontSize: 12, color: AppColors.textSecondary)) : const Icon(Icons.chevron_right, size: 18, color: AppColors.grey),
